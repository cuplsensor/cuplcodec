...
class MessageIntegrityError(UrlDecoderError):
...
class Decoder(MsgAuth):
    ...
    def __init__(self, encstr, secretkey):
        ...
        rem = smplcount % Decoder.SAMPLES_PER_OCTET
        full = int(smplcount / Decoder.SAMPLES_PER_OCTET)
    ...
    def samplesfromchunk(self, chunk, samplecount):
        chunksamples = list()
        decodedchunk = base64.urlsafe_b64decode(chunk)
        for i in range(0, (samplecount*2)-1, Decoder.BYTES_PER_SAMPLE):
...
if __name__ == '__main__':
    ...
    def testtime():
        timeintb64 = "ZZZ="
        print(HTDecoder().timeinterval(timeintb64))
