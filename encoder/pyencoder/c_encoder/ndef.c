...
#define TIMEINTB64_LEN  sizeof(timeintb64)-1
...
int ndef_writepreamble(int qlenblks, char * statusb64)
{
  ...
  int urllen = strlen(url)-1; // Get this from the nvparams.
  int urlbytes_remaining = urllen;  // The url bytes still to write.

  // Calculate message length
  int preurllen = URL_RECORD_HEADER_LEN + TLV_TYPE_LEN_LEN;
  const int posturllen_nopadding = TIMEINTB64_LEN + SERIALPARAM_LEN + VERPARAM_LEN + VERCHAR_LEN + STATPARAM_LEN + STATB64_LEN + QPARAM_LEN;
  int paddinglen = (BLKSIZE-1) - ((posturllen_nopadding + urllen + preurllen) & 0xF);
  int preamblenbytes = preurllen + urllen + posturllen_nopadding + paddinglen;
  ...
  while(urlbytes_remaining > 0) {
      blockremaining = (BLKSIZE-1)-eepindex; // Calculate remaining bytes to write in the current block.
      if (urlbytes_remaining > blockremaining)
      {
          eep_cp(&eepindex, url, blockremaining); // Fill the block.
          urlbytes_remaining -= blockremaining;
          eep_write(blk, 0); // Write block to EEPROM
          eepindex = 0; // Decrement eepindex.
      }
      else
      {
          eep_cp(&eepindex, url, urlbytes_remaining); // Fill the block.
