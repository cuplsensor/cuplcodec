 ...
 *
 * 2 octets are written from RAM buffer location 0 into the EEPROM block @ref _cursorblk.
 * 2 octets are written from RAM buffer location 1 into the EEPROM block @ref _nextblk.
 * @returns 0
 */
 ...
 * The function to modify the RAM buffer @ref eep_cp requires an index relative to @ref _cursorblk (it has no concept of octets).
