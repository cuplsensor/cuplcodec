 ...
 *
 * Sets counters to intial values and calls @see octet_read4 to read the first 4
 * octets into RAM.
 *
 ...
 *
 * 2 octets are written from RAM buffer location 0 into the EEPROM block @see _cursorblk.
 * 2 octets are written from RAM buffer location 1 into the EEPROM block @see _nextblk.
 * @returns 0
 */
 ...
 * The function to modify the RAM buffer @see eep_cp requires an index relative to @see _cursorblk (it has no concept of octets).
